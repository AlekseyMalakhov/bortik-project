...
function AdminEditBarCodeModal({ show, onHide, barcode, ...otherProps }) {
    ...
    const handleSubmit = (values) => {
        ...
        adminAPI
            .editBarcode(values, barcode.id)
            .then((response) => {
                dispatch(setLoading(false));
                dispatch(getBarcodes());
                if (response.status === 200) {
                    showAdminDoneModal("Штрихкод успешно отредактирован!");
                } else {
...
export default AdminEditBarCodeModal;
